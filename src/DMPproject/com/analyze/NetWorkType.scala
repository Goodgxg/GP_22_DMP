...
object NetWorkType {
  def main(args: Array[String]): Unit = {
    ...
    val lineData: RDD[(String, List[Double])] = dataRdd.map(row => {
      ...
      // 创建三个对应的方法处理九个指标
      (networkmannername, LogicUtils.request(requestmode, processnode) ++ LogicUtils.click(requestmode, iseffective) ++ LogicUtils.Ad(iseffective, isbilling, isbid, iswin, adorderid, WinPrice, adpayment))
    })
    ...
    SaveJDBCUtils.saveDataToMysql(saveDF,"t_network")
    spark.stop()
